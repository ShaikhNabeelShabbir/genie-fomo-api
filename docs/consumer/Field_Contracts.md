...
| Field | Route | Contract |
|---|---|---|
| `events[]`, `nextCursor`, `count`, `asOf`, `since`, `filters`, `note` | GET /events | keyset feed over `transactions`, `wallet_swaps` and `aum_samples` (`basis: sampled`), ordered `(at, kind, txHash \| handle, sub)` ascending, newest last (`sub` tells apart the transfer legs of ONE transaction — without it a page boundary that split a transaction dropped its remaining legs; a 3-part cursor issued before 19 Sep 2026 still decodes and re-delivers rather than loses); `since` defaults to now − 24 h; `limit` ≤ 500 (default 100). Every event carries `kind` (`transfer` \| `swap` \| `reading`), `at`, `handle`, `traderSource` (`traders.source`). `transfer`: `chain`, `tokenAddress`, `txHash`, `direction` (`in` \| `out`), `amount`, `counterparty`, `source` (`tx_source`), `txType`. `swap`: `chain`, `tokenAddress`, `txHash`, `tokenDelta`, `quoteDelta`, `quoteUsd`. `reading`: `totalUsd` (null = refused, never 0), `refusedReason`. `transfer` and `swap` carry `gates: { isHoneypot, canSell, priceSuspect: null }` from `token_info`, or `gates: null` when no row exists. Solana rows are real-time (webhook), EVM transfers nightly: poll with the cursor, not with `since` |
...
| Field | Route | Contract |
|---|---|---|
| `positions[].priceUsd`, `priceSource`, `pricedAt`, `valueUsd` | /positions, POST /traders/positions, /portfolio | Priced AT REQUEST TIME from one ladder, in order: `pegged` (quote_assets) → `token_price_stats` (the hourly DexScreener price, **no older than 24 h**) → `token_prices` (newest daily close **inside 7 days**) → `token_info` (GMGN, **fetched inside 7 days**). Since 19 Sep 2026 every rung has an age limit: a price past it prices nothing (`priceUsd`, `priceSource`, `pricedAt`, `valueUsd` are `null` and the row leaves the totals) — while DexScreener refused us for two days the top rung served a days-old price as current. `fomo_reported_entry` survives as a fallback when no rung carries a price, because the ladder cannot reproduce it. Until now these served the price frozen into `holdings` at the last balance read — a sweep that takes about nine hours — so a coin priced an hour ago still read `null`, and the same token showed three different prices on three traders' lists. |
