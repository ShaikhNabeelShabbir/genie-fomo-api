...
| **B3** where a same-day retry shows | fixed on branch, not yet deployed | Corrected 17 Sep: a retry inside the same hour overwrites the refused reading (the readings table keys on the hour), so no route can show it, and sampling stopped on 17 Sep 2026 ~03:52 UTC. `/aum/history` hours with nothing priceable carry a `reason` word instead. Closed. | `GET $BASE/v1/traders/gmgn_8gv31ab8mt/aum?window=1w&step=1h&live=false` → `count` equals the readings held, `bucketMs: 3600000` |
