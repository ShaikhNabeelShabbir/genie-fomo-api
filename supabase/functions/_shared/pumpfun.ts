 ...
 * TWIN OF scripts/lib/pumpfun.mjs: edit both.
 */
import { base58Decode, findProgramAddress } from "./solana_pda.ts";
...
export interface Curve { readonly progress: number; readonly graduated: boolean }

export async function bondingCurveAddress(mint: string): Promise<string> {
  return (await findProgramAddress([new TextEncoder().encode("bonding-curve"), base58Decode(mint)], PUMP_PROGRAM)).address;
}

/** Decode a BondingCurve account. Null when the bytes are not one. */
export function decodeCurve(bytes: Uint8Array): Curve | null {
