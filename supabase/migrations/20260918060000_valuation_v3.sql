-- Valuation v3 (17 Sep 2026, fix request v3 items V1 / N1 / Z1b). Both SQL valuers,
-- aum_live_refresh (20260918030000) and aum_history_build (20260918040000), re-created with
-- the same bodies and these changes only:
--   1. a price <= 0 is unpriced at every rung (Z1b: a $0 GMGN impostor was "priced");
--   2. aum_live_refresh gains the token_prices rung (latest day within 7 days) between
--      token_price_stats and token_info (N1: native ETH/BNB live only in token_prices);
--   3. honeypot rows (token_info.is_honeypot or can_not_sell) leave the total for unsellable_usd;
--   4. the suspect rule from aum-sample/value.ts runs per row, after the three ceilings;
--      suspect value goes to suspect_usd, never into total_usd (V1);
--   5. priced_positions counts value_usd > 0 that is neither suspect nor unsellable; total_usd
--      is stored UNROUNDED (the route rounds); nothing counted with suspect rows -> price_suspect.
...
--   CONCENTRATION_SHARE = 0.9, CONCENTRATION_TOTAL_USD = 1e9.
-- Floors mirror supabase/functions/api/shared/aum-rules.ts: PRICED_FLOOR = 0.25, PARTIAL_SERVE_FLOOR_USD = 100.

alter table aum_live
  add column if not exists suspect_usd    numeric,
  add column if not exists unsellable_usd numeric;
alter table aum_history
  add column if not exists suspect_usd    numeric,
  add column if not exists unsellable_usd numeric;

comment on column aum_live.suspect_usd is
  'Gross value of positions whose price failed the value.ts suspect rule; excluded from total_usd. Null: none.';
comment on column aum_live.unsellable_usd is
  'Gross value of honeypot / cannot-sell positions; excluded from total_usd. Null: none.';
comment on column aum_history.suspect_usd is
  'Gross value of positions whose price failed the value.ts suspect rule; excluded from total_usd. Null: none (always null on basis reading).';
comment on column aum_history.unsellable_usd is
  'Gross value of honeypot / cannot-sell positions; excluded from total_usd. Null: none (always null on basis reading).';

alter table aum_live drop constraint if exists aum_live_reason_check;
alter table aum_live add constraint aum_live_reason_check
  check (reason in ('no_holdings','no_prices','too_little_priced','price_suspect'));

alter table aum_history drop constraint if exists aum_history_reason_check;
alter table aum_history add constraint aum_history_reason_check
  check (reason in ('no_holdings','no_prices','too_little_priced','price_suspect'));
