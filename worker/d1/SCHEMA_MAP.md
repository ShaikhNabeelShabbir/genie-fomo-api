...
| `aum_history_build(p_handle, p_from, p_to)` (SQL) | `20260918010000` → `…040000` → `…060000` → **final body `20260918100000_valuation_v4_no_market.sql`** | **`buildAumHistory(sql, handle, fromIso, toIso)` in `worker/src/jobs/valuation.ts`** (called by `jobs/aum_history.ts`). TS over bounded D1 reads: the hour series, reading-per-hour, capture-per-(hour, network), price ladder (peg → hourly ≤ 24 h → daily → token_info current hour), the value.ts classification per hour and the upsert into `aum_history`. |
| `aum_live_refresh(p_handles, p_source, p_older_than)` (SQL) | `20260918030000` → `…060000` → **final body `20260918100000_valuation_v4_no_market.sql`** | **`refreshAumLive(sql, handles, source, olderThanHours?)` in `worker/src/jobs/valuation.ts`** (called by `jobs/aum_history.ts` and `jobs/aum_live_flush.ts`; `webhook.ts` only marks `aum_live_dirty`). Same valuation over `holdings_live`, price ladder peg → `token_price_stats` → `token_prices` ≤ 7 days → `token_info`, upserting `aum_live` and the current hour of `aum_history`. |
...
| `md5(...)` in the `transactions.transfer_key` digest | `20260904120000` | `transferKey()` in `supabase/functions/_shared/md5.ts` (SQLite has no md5 and the Workers Web Crypto has no MD5), computed before the insert in `jobs/transfers.ts` and `webhook.ts`. |
